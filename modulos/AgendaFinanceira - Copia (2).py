...
def extrair_id_seguro(valor_raw):
    """Resgata o ID real caso o st_searchbox retorne Dicionário JSON."""
    if isinstance(valor_raw, dict):
        if 'value' in valor_raw and valor_raw['value'] is not None:
            return valor_raw['value']
        if 'result' in valor_raw and valor_raw['result'] is not None:
            return valor_raw['result']
        for v in valor_raw.values():
            try:
                return int(v)
...
def callback_salvar_lancamento(acao="inserir", id_lancamento=None):
    ...
    if modo_pag == "Cartão de Crédito":
        ...
        if id_cc_final:
            _, _, _, df_cc = obter_auxiliares_leves()
...
def modal_formulario(acao="inserir", id_lancamento=None, dados_pre=None):
    ...
    df_class, _, _, df_cc = obter_auxiliares_leves()
...
def modal_baixa(id_l, ev_nome, v_orig):
    fr_id = st.session_state.get("form_reset", 0)
    _, df_bancos, df_contas, _ = obter_auxiliares_leves()
...
def renderizar_painel_filtros():
    with st.container(border=True):
        ...
        df_ev_list = GerenciadorBanco.executar_query("SELECT nome FROM eventos ORDER BY nome ASC")
...
if not df.empty:
    for _, row in df.iterrows():
        ...
        if b64: 
            html_i = f"<img src='data:image/png;base64,{b64}' style='width: 52px; mix-blend-mode: multiply; margin-right: 15px;' />"
