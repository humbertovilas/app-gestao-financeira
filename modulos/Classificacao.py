...
def modal_inclusao():
    ...
    nome = st.text_input("Nome da classificação:", placeholder="Ex: Despesas da residência...")
    cat_pai = st.selectbox("Vincular à categoria pai:", options=cats.values.tolist(), format_func=lambda x: x[1])
    st.write("")
...
def modal_alteracao(id_class, nome_atual, id_cat_atual):
    nome = st.text_input("Nome da classificação:", value=nome_atual)
    cats = GerenciadorBanco.executar_query("SELECT id, nome FROM categorias ORDER BY nome")
    cat_list = cats.values.tolist(); idx_cat = next((i for i, c in enumerate(cat_list) if c[0] == id_cat_atual), 0)
    cat_pai = st.selectbox("Vincular à categoria pai:", options=cat_list, format_func=lambda x: x[1], index=idx_cat)
    st.write("")
...
if df.empty:
    ...
    with st.container():
        ...
        for _, row in df.iterrows():
            ...
            if c3.button(" ", icon=":material/edit:", key=f"ed_cl_{row['id']}", help="Editar", use_container_width=True): modal_alteracao(row['id'], row['nome'], row['id_categoria'])
            if c4.button(" ", icon=":material/content_copy:", key=f"cp_cl_{row['id']}", help="Duplicar", use_container_width=True): modal_duplicacao(row['nome'], row['id_categoria'])
            if c5.button(" ", icon=":material/delete:", key=f"del_cl_{row['id']}", help="Excluir", use_container_width=True): modal_exclusao(row['id'], row['nome'])
