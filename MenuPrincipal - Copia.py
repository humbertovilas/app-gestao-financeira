...
# ROTINA DE LOGOFF NATIVA DO ROTEADOR
def rotina_sair():
    st.session_state.autenticado = False
    st.session_state.usuario_logado = ""
    st.session_state.email_logado = ""
...
def iniciar_sistema():
    ...
    if not st.session_state.autenticado:
        ...
        # O NOVO BLOCO UNIFICADO (PAINEL DE CONTROLE PESSOAL)
        paginas_pessoal = [
            st.Page("modulos/MeuPerfil.py", title="Meu perfil", icon=":material/person:")
        ]
        
        if st.session_state.perfil_logado == "Administrador":
            paginas_pessoal.append(st.Page("modulos/CadastroUsuario.py", title="Gestão de usuários", icon=":material/manage_accounts:"))
        
        paginas_pessoal.append(st.Page(rotina_sair, title="Sair", icon=":material/logout:"))
        
        dicionario_paginas = {
            "Operação e cadastros": paginas_operacao,
            "Tabelas auxiliares": paginas_auxiliares,
            "Painel pessoal": paginas_pessoal
        }
